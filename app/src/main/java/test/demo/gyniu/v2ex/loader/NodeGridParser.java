...
public class NodeGridParser extends ParserHelper {
    public static List<Node> parseFavNodes(Document doc) {
        ...
        return Lists.newArrayList(Iterables.transform(elements, new Function<Element, Node>() {
            @Override
            public Node apply(Element input) {
                return parseNode(input);
            }
        }));
    }

    private static Node parseNode(Element ele) {
